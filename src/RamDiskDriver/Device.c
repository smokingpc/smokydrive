    ...
    )
{
    ...
    //AssignName要放前面，不然CreateDevice後DevInit會變null (不明)
    status = RegisterRamdiskDeviceName(DeviceInit);
    if (!NT_SUCCESS(status))
        KdPrint(("==[SmokyDrive] RegisterRamdiskDeviceName() failed 0x%08X", status));
...
//        if (irp) {
//            irp->AssociatedIrp.SystemBuffer = mntName;
//            PIO_STACK_LOCATION stack = IoGetNextIrpStackLocation(irp);
//            stack->Parameters.DeviceIoControl.InputBufferLength = mntNameLength;
//
